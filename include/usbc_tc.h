...
/**
 * @brief From entry to Attached.SRC until VBUS reaches the minimum vSafe5V
 *threshold as measured at the source’s receptacle See Table 4-29 VBUS and VCONN
 *Timing Parameters
 */
#define TC_T_VBUS_ON_MAX_MS 275

/**
 * @brief From the time the Sink is detached until the Source removes VBUS and
 *reaches vSafe0V (See USB PD). See Table 4-29 VBUS and VCONN Timing Parameters
 */
...
/**
 * @brief From the time a Sink with accessory support enters the
 *PoweredAccessory state until the Sink sources minimum VCONN voltage (see Table
 *4-5) See Table 4-29 VBUS and VCONN Timing Parameters
 */
#define TC_T_VCONN_ON_PA_MAX_MS 100

/**
 * @brief From the time that a Sink is detached or as directed until the VCONN
 *supply is disconnected. See Table 4-29 VBUS and VCONN Timing Parameters
 */
#define TC_T_VCONN_OFF_MAX_MS 35

/**
 * @brief Response time for a Sink to adjust its current consumption to be in
 *the specified range due to a change in USB Type-C Current advertisement See
 *Table 4-29 VBUS and VCONN Timing Parameters
 */
#define TC_T_SINK_ADJ_MAX_MS 60

/**
 * @brief The minimum period a DRP shall complete a Source to Sink and back
 *advertisement See Table 4-30 DRP Timing Parameters
 */
#define TC_T_DRP_MIN_MS 50

/**
 * @brief The maximum period a DRP shall complete a Source to Sink and back
 *advertisement See Table 4-30 DRP Timing Parameters
 */
#define TC_T_DRP_MAX_MS 100

/**
 * @brief The minimum time a DRP shall complete transitions between Source and
 *Sink roles during role resolution See Table 4-30 DRP Timing Parameters
 */
#define TC_T_DRP_TRANSITION_MIN_MS 0

/**
 * @brief The maximum time a DRP shall complete transitions between Source and
 *Sink roles during role resolution See Table 4-30 DRP Timing Parameters
 */
...
/**
 * @brief Minimum Time for a DRP to detect that the connected Charge-Through
 *VCONNPowered USB Device has been detached, after VBUS has been removed. See
 *Table 4-30 DRP Timing Parameters
 */
#define TC_T_VPD_DETACH_MIN_MS 10

/**
 * @brief Maximum Time for a DRP to detect that the connected Charge-Through
 *VCONNPowered USB Device has been detached, after VBUS has been removed. See
 *Table 4-30 DRP Timing Parameters
 */
...
/**
 * @brief Minimum time a Sink port shall wait before it can determine it is
 *detached due to the potential for USB PD signaling on CC as described in the
 *state definitions. See Table 4-31 CC Timing
 */
#define TC_T_PD_DEBOUNCE_MIN_MS 10

/**
 * @brief Maximum time a Sink port shall wait before it can determine it is
 *detached due to the potential for USB PD signaling on CC as described in the
 *state definitions. See Table 4-31 CC Timing
 */
...
/**
 * @brief Minimum Time a port shall wait before it can determine it is
 *re-attached during the try-wait process. See Table 4-31 CC Timing
 */
#define TC_T_TRY_CC_DEBOUNCE_MIN_MS 10

/**
 * @brief Maximum Time a port shall wait before it can determine it is
 *re-attached during the try-wait process. See Table 4-31 CC Timing
 */
#define TC_T_TRY_CC_DEBOUNCE_MAX_MS 10

/**
 * @brief Minimum time a self-powered port shall remain in the ErrorRecovery
 *state. See Table 4-31 CC Timing
 */
#define TC_T_ERROR_RECOVERY_SELF_POWERED_MIN_MS 25

/**
 * @brief Minimum time a source shall remain in the ErrorRecovery state if it
 *was sourcing VCONN in the previous state. See Table 4-31 CC Timing
 */
#define TC_T_ERROR_RECOVERY_SOURCE_MIN_MS 240

/**
 * @brief Minimum time a Sink port shall wait before it can determine there has
 *been a change in Rp where CC is not BMC Idle or the port is unable to detect
 *BMC Idle. See Table 4-31 CC Timing
 */
#define TC_T_RP_VALUE_CHANGE_MIN_MS 10

/**
 * @brief Maximum time a Sink port shall wait before it can determine there has
 *been a change in Rp where CC is not BMC Idle or the port is unable to detect
 *BMC Idle. See Table 4-31 CC Timing
 */
...
/**
 * @brief Minimum time a Source shall detect the SRC.Open state. The Source
 *should detect the SRC.Open state as quickly as practical. See Table 4-31 CC
 *Timing
 */
#define TC_T_SRC_DISCONNECT_MIN_MS 0

/**
 * @brief Maximum time a Source shall detect the SRC.Open state. The Source
 *should detect the SRC.Open state as quickly as practical. See Table 4-31 CC
 *Timing
 */
...
/**
 * @brief Minimum time to detect connection when neither Port Partner is
 *toggling. See Table 4-31 CC Timing
 */
#define TC_T_NO_TOGGLE_CONNECT_MIN_MS 0

/**
 * @brief Maximum time to detect connection when neither Port Partner is
 *toggling. See Table 4-31 CC Timing
 */
...
/**
 * @brief Minimum time for a Charge-Through VCONN-Powered USB Device to detect
 *that the Charge-Through source has disconnected from CC after VBUS has been
 *removed, transition to CTUnattached.VPD, and re-apply its Rp termination
 *advertising 3.0 A on the host port CC. See Table 4-31 CC Timing
 */
#define TC_T_VPDCTDD_MIN_US 30

/**
 * @brief Maximum time for a Charge-Through VCONN-Powered USB Device to detect
 *that the Charge-Through source has disconnected from CC after VBUS has been
 *removed, transition to CTUnattached.VPD, and re-apply its Rp termination
 *advertising 3.0 A on the host port CC. See Table 4-31 CC Timing
 */
...
/**
 * @brief Minimum time for a Charge-Through VCONN-Powered USB Device shall
 *remain in CTDisabled.VPD state. See Table 4-31 CC Timing
 */
