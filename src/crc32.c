...
void crc32_ctx_init(uint32_t *crc) { *crc = CRC32_INITIAL; }
...
uint32_t crc32_ctx_result(uint32_t *crc) { return *crc ^ 0xFFFFFFFF; }

/* Accumulator for the CRC */
static uint32_t crc_;

void crc32_init(void) { crc32_ctx_init(&crc_); }

void crc32_hash(const void *buf, int size) { crc32_ctx_hash(&crc_, buf, size); }

void crc32_hash32(uint32_t val) { crc32_ctx_hash32(&crc_, val); }

void crc32_hash16(uint16_t val) { crc32_ctx_hash16(&crc_, val); }

uint32_t crc32_result(void) { return crc32_ctx_result(&crc_); }
