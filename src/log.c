...
static const char * unstructured_vdm_command_str =
	"CUSTOM\t\t";
...
static void log_vdm(FILE *f)
{
	...
	if (v.type == 1) {
		fprintf(f, "[%d]%s:%s", msgid, data_msg_str[data_msg], structured_vdm_command_str[v.command]);
	} else {
		fprintf(f, "[%d]%s:%s", msgid, data_msg_str[data_msg], unstructured_vdm_command_str);
	}
}
...
void log_file(uint8_t *buf) {
	...
	uint32_t timer = curTime.tv_usec / 1000 + 1000 * (tm->tm_sec + 60 * (tm->tm_min + 60 * (tm->tm_hour + 24 * tm->tm_mday)));
